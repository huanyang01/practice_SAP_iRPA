...
POPUPS.step({ Read_an_XML_file: function(ev, sc, st) {
	ctx.log("Start: --- Step: Read_an_XML_file");
	var rootData = sc.data;
	...
	ctx.log("End: --- Step: Read_an_XML_file");
	return;
}});
...
POPUPS.step({ Write_log: function(ev, sc, st) {
	...
	ctx.log("Config: "+rootData.Config.root.AttachmentFilePath.Excel, e.logIconType.Info);
...
POPUPS.step({ Open_existing_Excel_f: function(ev, sc, st) {
	...
	ctx.excel.file.open(rootData.Config.root.AttachmentFilePath.Excel+"\\Reminder0627.xlsx");
