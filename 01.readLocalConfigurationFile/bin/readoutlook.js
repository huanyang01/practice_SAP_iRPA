...
// ----------------------------------------------------------------
//   Test menu for scenario ReadOutlook 
// ----------------------------------------------------------------
GLOBAL.events.START.on(function (ev) {
	if (ctx.options.isDebug) {
		// Add item in systray menu.
		systray.addMenu('', 'ReadOutlook', 'Test ReadOutlook', '', function (ev) {
			var rootData = ctx.dataManagers.rootData.create();
			
			// Initialize your data here.
			POPUPS.scenarios.ReadOutlook.start(rootData);
		});
	}
});

//---------------------------------------------------
// Scenario ReadOutlook Starter ()
//---------------------------------------------------

// ----------------------------------------------------------------
//   Scenario: ReadOutlook
// ----------------------------------------------------------------
POPUPS.scenario({ ReadOutlook: function(ev, sc) {
...
POPUPS.step({ Read_an_XML_file: function(ev, sc, st) {
	var rootData = sc.data;
	ctx.workflow('ReadOutlook', '255b6003-6d71-46a2-beca-e073a8a56acd') ;
...
POPUPS.step({ Write_log: function(ev, sc, st) {
	var rootData = sc.data;
	ctx.workflow('ReadOutlook', '1e9861c3-93dd-47ae-9e13-1056a699d9d0') ;
...
POPUPS.step({ Initialize_Excel: function(ev, sc, st) {
	var rootData = sc.data;
	ctx.workflow('ReadOutlook', '7259d4d3-2a16-4728-b9dc-e26f1543cff8') ;
...
POPUPS.step({ Open_existing_Excel_f: function(ev, sc, st) {
	var rootData = sc.data;
	ctx.workflow('ReadOutlook', 'fa3dd3b8-fae3-4231-86b2-b7b6d9cecc90') ;
