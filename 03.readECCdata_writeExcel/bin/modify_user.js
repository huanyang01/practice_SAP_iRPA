...
// ----------------------------------------------------------------
//   Test menu for scenario Modify_User 
// ----------------------------------------------------------------
GLOBAL.events.START.on(function (ev) {
	if (ctx.options.isDebug) {
		// Add item in systray menu.
		systray.addMenu('', 'Modify_User', 'Test Modify_User', '', function (ev) {
			var rootData = ctx.dataManagers.rootData.create();
			
			// Initialize your data here.
			SAPGUI.scenarios.Modify_User.start(rootData);
		});
	}
});

//---------------------------------------------------
// Scenario Modify_User Starter ()
//---------------------------------------------------

// ----------------------------------------------------------------
//   Scenario: Modify_User
// ----------------------------------------------------------------
SAPGUI.scenario({ Modify_User: function(ev, sc) {
...
SAPGUI.step({ pSAPEasyAccessU_manag: function(ev, sc, st) {
	var rootData = sc.data;
	ctx.workflow('Modify_User', 'f392a7ec-9ca4-4cc1-b0d4-3472f9dd95a6') ;
...
SAPGUI.step({ Initialize_Excel: function(ev, sc, st) {
	var rootData = sc.data;
	ctx.workflow('Modify_User', '6f2b0e07-bf43-4128-93d7-9583c26de250') ;
...
SAPGUI.step({ Open_existing_Excel_f: function(ev, sc, st) {
	var rootData = sc.data;
	ctx.workflow('Modify_User', '4d9b113b-9795-41bb-8fa5-ab7e4631622c') ;
...
SAPGUI.step({ Set_one_value: function(ev, sc, st) {
	var rootData = sc.data;
	ctx.workflow('Modify_User', 'a6143909-2516-4301-abee-4b83aa6aaad2') ;
...
SAPGUI.step({ pUserMaintenanceIni_m: function(ev, sc, st) {
	var rootData = sc.data;
	ctx.workflow('Modify_User', '90e6c067-c1c9-4afa-b918-0e143c166d98') ;
	// Wait until the Page loads
