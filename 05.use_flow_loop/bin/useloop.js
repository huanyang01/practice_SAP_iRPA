...
GLOBAL.step({ Set_context: function(ev, sc, st) {
	...
	index = 3;	
	ctx.log('Log: Index value is : ' + index);
...
GLOBAL.step({ Start_loop: function(ev, sc, st) {
	ctx.log('Log: Start --- Step: Start_loop');
	var rootData = sc.data;
	ctx.log('Log: Start --- Step: Start_loop - before ctx.workflow');
	ctx.workflow('useLoop', '4bd1bc52-5a3d-4a48-90a0-ae292179588a') ;
	ctx.log('Log: Start --- Step: Start_loop - after ctx.workflow');
	// Starting instruction for a loop.
	if (sc.localData.Startloop < 0) sc.localData.Startloop = 0;
	ctx.log('Log: Start --- Step: Start_loop - before sc.endStep');
	sc.endStep(); // Exit_loop
	ctx.log('Log: End - Step: Start_loop');
	return;
}});
...
GLOBAL.step({ Exit_loop: function(ev, sc, st) {
	ctx.log('Log: Start --- Step: Exit_loop');
	var rootData = sc.data;
	...
	ctx.log('Log: End - Step: Exit_loop');
	return;
}});
...
GLOBAL.step({ Write_log: function(ev, sc, st) {
	...
	ctx.log('Step: Write_log - In the loop: ' + index, e.logIconType.Info);
...
GLOBAL.step({ Loops_to_the_start_bl: function(ev, sc, st) {
	ctx.log('Log: Start --- Step: Loops_to_the_start_bl');
	var rootData = sc.data;
	...
	ctx.log('Log: End - Step: Loops_to_the_start_bl');
	return;
}});
